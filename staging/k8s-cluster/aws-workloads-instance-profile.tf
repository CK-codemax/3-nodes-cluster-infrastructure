# Combined IAM Role for all AWS workloads (for self-managed clusters using instance profiles)
...
resource "aws_iam_role" "aws_workloads_instance_profile" {
  ...
  tags = {
    ...
    Description = "Combined IAM role for all AWS workloads (self-managed cluster)"
  }
}
